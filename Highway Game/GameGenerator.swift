...
class GameGenerator {
    ...
    func createCarRow(laneAmount: Int = GameTools.currentGameLaneAmount, startLane: Int, endLane: Int, holeAmount: Int, yPosition: CGFloat) -> [GameObstacle] {
        ...
        let holeLaneNumber = Int.random(in: startLane...(startLane + (endLane - totalHoles) + 1)) //lane number of where there isn't a car
    ...
    func generateChunk(sceneYPosition: CGFloat) -> (obstacles: [GameObstacle], chunkSize: Int) {
        ...
        if(GameTools.currentGameMode == GameMode.Freeplay) {
        ...
        else if(GameTools.currentGameMode == GameMode.TripleLane) {
            let chunkType = Int.random(in: 0...1)
            
            if(chunkType == 0) {
                chunkSize = 800
                
                obstacles += createCarRow(startLane: 0, endLane: 2, holeAmount: 1, yPosition: sceneYPosition)
                
                obstacles += createCarRow(startLane: 0, endLane: 2, holeAmount: 1, yPosition: sceneYPosition + 300)
                
                obstacles += createCarRow(startLane: 0, endLane: 2, holeAmount: 1, yPosition: sceneYPosition + 600)
                
                obstacles.append(createGameObstacle(type: ObstacleType.Cone, lane: 0, y: sceneYPosition + 900))
                obstacles.append(createGameObstacle(type: ObstacleType.Cone, lane: 2, y: sceneYPosition + 900))
            }
            else if(chunkType == 1) {
                chunkSize = 800
                
                obstacles.append(createGameObstacle(type: ObstacleType.Cone, lane: 1, y: sceneYPosition))
                obstacles.append(createGameObstacle(type: ObstacleType.Cone, lane: 2, y: sceneYPosition))
                
                obstacles.append(createGameObstacle(type: ObstacleType.Cone, lane: 0, y: sceneYPosition + 300))
                obstacles.append(createGameObstacle(type: ObstacleType.Cone, lane: 1, y: sceneYPosition + 300))
                
                obstacles.append(createGameObstacle(type: ObstacleType.Cone, lane: 0, y: sceneYPosition + 600))
                obstacles.append(createGameObstacle(type: ObstacleType.Cone, lane: 2, y: sceneYPosition + 600))
                
                obstacles.append(createGameObstacle(type: ObstacleType.Cone, lane: 1, y: sceneYPosition + 900))
                obstacles.append(createGameObstacle(type: ObstacleType.Cone, lane: 2, y: sceneYPosition + 900))
            }
        }
